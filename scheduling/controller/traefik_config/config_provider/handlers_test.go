			...
			setupCurrentConfig: &TraefikDynamicConfiguration{
				HTTP: &HTTPConfiguration{
					Routers: map[string]Router{
						"router-test-1": {Rule: "Host(`example.com`)", Service: "service-test-1"},
					},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.checkEmptyConfig {
				// For empty config test, we expect a structure with all maps initialized but empty
				...
				if !reflect.DeepEqual(&gotBody, expectedEmptyBody) {
					t.Errorf("Handler returned unexpected empty configuration when config was nil: \ngot %#v, \nexpected %#v", gotBody, expectedEmptyBody)
				}
			} else if tt.wantBody != nil {
				// For non-empty config test, directly compare parsed structs
