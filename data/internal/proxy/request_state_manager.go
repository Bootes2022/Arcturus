...
func (m *RequestStateManager) AddState(state *RequestState) {
	...
	log.Printf("[StateManager] : ID=%d, =%s", state.RequestID, state.Status)
}
...
func (m *RequestStateManager) UpdateStatus(requestID uint32, newStatus RequestStatus) bool {
	...
	if !exists {
		log.Printf("[StateManager] : : ID=%d", requestID)
		return false
	}
	...
	log.Printf("[StateManager] : ID=%d, %s -> %s", requestID, oldStatus, newStatus)
	return true
}
...
func (m *RequestStateManager) doCleanup() {
	...
	if len(expiredIDs) > 0 {
		...
		log.Printf("[StateManager]  %d ", len(expiredIDs))
	}
}
