...
func (w *TaskWorker) Start(ctx context.Context) error {
	...
	log.Printf("[%s] Registered processors for task types:", w.workerID)
	for taskType := range w.processors {
		log.Printf("[%s] - %s", w.workerID, taskType)
	}
	...
	for {
		select {
		case <-ctx.Done():
			log.Printf("[%s] Worker shutting down...\n", w.workerID)
			return nil
...
func (w *TaskWorker) handleTaskEvent(ctx context.Context, event *clientv3.Event) {
	...
	if err := json.Unmarshal(event.Kv.Value, &task); err != nil {
		log.Printf("[%s] Failed to unmarshal task: %v\n", w.workerID, err)
		return
	}
	...
	if !ok {
		log.Printf("[%s] No processor registered for task type: %s\n", w.workerID, task.Type)
		return
	}
	...
	if err != nil {
		log.Printf("[%s] Failed to update task status: %v\n", w.workerID, err)
		return
	}
	...
	if err != nil {
		...
		log.Printf("[%s] Task processing failed: %s - %v\n", w.workerID, task.ID, err)
	} else {
		task.Status = "completed"
		taskResult.Result = result
		log.Printf("[%s] Task completed successfully: %s - Result: %s\n", w.workerID, task.ID, result)
	}
	...
	if err != nil {
		log.Printf("[%s] Failed to store task result: %v\n", w.workerID, err)
		return
	}
	...
	_, err = w.client.Put(ctx, string(event.Kv.Key), string(taskJSON))
	if err != nil {
		log.Printf("[%s] Failed to update task status after completion: %v\n", w.workerID, err)
		return
	}
}
