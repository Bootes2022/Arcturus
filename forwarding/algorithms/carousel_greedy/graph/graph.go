...
// Path represents a directed path in the graph
type Path struct {
	Nodes   []int   // Sequence of nodes in the path
	Flow    float64 // Bottleneck flow of the path
	Latency float64 // Total latency of the path
}

// Copy creates a deep copy of a path
...
// UpdateEdgeUsage increases the usage count for all edges in the path
...
// GetPathLatency calculates the total latency of a path
...
// GetMaxFlow calculates the maximum possible flow through a path
