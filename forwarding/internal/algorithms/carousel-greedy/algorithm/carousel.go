...
func CarouselGreedy(g *graph.Graph, thetaA, thetaL float64, maxEdgeUsage, alpha, beta int) []*graph.Path {
	logger.LogSectionStart("Carousel Greedy")
	logger.Info("Carousel Greedy，: %.2f, : %.2f, : %d, Alpha: %d, Beta: %d",
	...
	logger.Info(" %d ", len(initialSolution))
	...
	if beta > 0 && len(currentSolution) > beta {
		...
		for _, path := range removedPaths {
			//logger.LogPathRemoval(path, "Beta")
			// resetPathFlow，
	...
	logger.Info("Carousel Greedy， %d ，: %.2f",
		len(bestSolution), CalculateTotalFlow(bestSolution))
	logger.LogSectionEnd("Carousel Greedy")
