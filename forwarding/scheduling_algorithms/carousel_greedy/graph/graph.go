...
// Path represents a directed router in the graph
type Path struct {
	Nodes   []int   // Sequence of nodes in the router
	Flow    float64 // Bottleneck flow of the router
	Latency float64 // Total latency of the router
}

// Copy creates a deep copy of a router
...
// UpdateEdgeUsage increases the usage count for all edges in the router
...
// GetPathLatency calculates the total latency of a router
...
// GetMaxFlow calculates the maximum possible flow through a router
