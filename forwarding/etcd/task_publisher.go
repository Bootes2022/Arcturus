...
func (p *TaskPublisher) PublishTask(ctx context.Context, task Task) error {
	...
	log.Printf("[%s] Task published: %s (Type: %s)\n", p.publisherID, task.ID, task.Type)
	return nil
}
			...
			for _, event := range resp.Events {
				if event.Type == clientv3.EventTypePut {
					...
					if err := json.Unmarshal(event.Kv.Value, &result); err != nil {
						log.Printf("Failed to unmarshal task result: %v\n", err)
						continue
					}
...
func (p *TaskPublisher) Run(ctx context.Context) error {
	log.Printf("[%s] Publisher started\n", p.publisherID)
	...
	log.Printf("[%s] Waiting for result of task: %s\n", p.publisherID, mathTask.ID)
	result, err := p.WaitForResult(ctx, mathTask.ID, 30*time.Second)
	if err != nil {
		log.Printf("[%s] Error: %v\n", p.publisherID, err)
	} else {
		log.Printf("[%s] Math task completed. Result: %s, Error: %s\n",
	...
	if err != nil {
		...
		log.Printf("[%s] String task completed. Result: %s, Error: %s\n",
			p.publisherID, result.Result, result.Error)
	}
